...
class MPSNode:
    ...
    def find_neighbor(self,j):
        ...
        if(len(re) == 0):
            ...
            raise RuntimeError("find_neighbor: two positions storing the same neighbor")
        else:
            return re[0][0]
    ...
    def move2tail(self,idx):
        ...
        error = 0
        if(idx<0):
            raise ValueError("move2tail(): idx should be >= 0")
    ...
    def move(self,a,b):
        ...
        if(a==b):
            return error
        if(a<0 or b<0):
            raise ValueError("move(): indices should be >= 0")
        if(b>a):
            b = b
            for i in range(a,b):
                error = error + self.swap(i,i+1)
...
#        print("in swap(), move cano")
        ...
        if(abs(i-j) != 1):
            raise ValueError(f"swap(): i={i} and j={j} must be consecutive indices")
    ...
    def merge(self,j,cross=False):
        ...
        shape = self.shape()
        if(idxj.size != 2):
            raise RuntimeError(f"merge(): expected exactly 2 occurrences of neighbor {j}, got {idxj.size}")
        idx1 = idxj[0][0]
        idx2 = idxj[1][0]
...
#
        ...
        if( len(nodej.mps) == 1): # node i is not a leaf, j is a leaf
            ...
            if(self.norm_method == 1):
                ...
                raise ValueError(f"eat(): unknown norm_method={self.norm_method}")
        ...
        if(len(self.mps) > 1):
            ...
            raise RuntimeError("eat(): unexpected single-site MPS state during contraction")
...
#            print("this should be the last tensor",z)
        ...
        raise RuntimeError("lognorm(): multi-site MPS norm computation is not supported in contraction")
